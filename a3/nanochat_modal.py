...
Individual stages (if you want to re-run one step):
    ...
    modal run nanochat_modal.py::stage_final_report
...
app = modal.App("nanochat-d24-speedrun-base")
...
volume = Volume.from_name("nanochat-d24-vol-base", create_if_missing=True)
...
@app.function(
    ...
    secrets=[secret],
    volumes={VOLUME_MOUNT: volume},
    timeout=60 * 5,  # 5 minutes is more than enough for report generation
)
def stage_final_report() -> None:
    """
    speedrun.sh:
    ...
    # speedrun.sh: python -m nanochat.report generate
    _python("nanochat.report", ["generate"])
    
    volume.commit()
    print("Final report generated successfully inside the Modal volume.")
...
def main() -> None:
    ...
    #              torchrun ... -m scripts.chat_eval -- -i sft
    print("[4/6] Supervised fine-tuning + eval...")
    stage_sft.remote(wandb_run=WANDB_RUN)

    # Stage 5: Final Report
    # speedrun.sh: python -m nanochat.report generate
    print("[5/6] Generating final report...")
    stage_final_report.remote()
